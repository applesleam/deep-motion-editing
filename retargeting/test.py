...
if __name__ == '__main__':
    test_characters = ['AJ_m', 'Big_Vegas_m', 'Kaya_m', 'Mousey_m', 'Ortiz_m']
    ...
    cross_dest_path = pjoin(prefix, 'results_XHY_infer/cross_structure/')
    intra_dest_path = pjoin(prefix, 'results_XHY_infer/intra_structure/')
    source_path = pjoin(prefix, 'results_XHY_infer/bvh/')

    cross_error = []
    intra_error = []
    for i in range(5):
        print('Batch [{}/5]'.format(i + 1))
        # eval(i, prefix)
    ...
    os.system('rm -r %s' % pjoin(prefix, 'results_XHY_infer/bvh'))
